...
//				callback('stalled',_currentTime);
			...
			this.currentTimes=function(){
				return Math.floor(_currentTime);
			};
		}(_callback);		
...
//				obj.addEventListener('ended',onEnded)
		...
		var Video_Content = function(dom,list,id){
			var ontimeupdate=function(e){
				if(e.target._currentTime){return}
...
//				mv.removeEventListener('canplay'});
			...
			this.seek=function(time){
				...
				if(this.index == i){
					this.dom.childNodes[this.activeI].currentTime = time -(i!=0?this.show_time[i-1]:0);this.index = i;return;	
				}
				if(this.index == i-1){
					this.activeI = 1-this.activeI;
					this.dom.childNodes[this.activeI].currentTime = time -(i!=0?this.show_time[i-1]:0);
					this.dom.childNodes[this.activeI].style.display = 'block';
					this.dom.childNodes[this.activeI].volume=this.dom.childNodes[0].volume;
					this.dom.childNodes[this.activeI].muted=this.dom.childNodes[0].muted;
					this.dom.childNodes[1-this.activeI].style.display = 'none';
					if(i<this.list.length-1&&this.index != i){
						this.dom.childNodes[1-this.activeI].src = this.list[i+1];
					};
					this.index = i;
					return;	
				}
				this.dom.childNodes[this.activeI].src=this.list[i];
				this.dom.childNodes[this.activeI]._currentTime = time -(i!=0?this.show_time[i-1]:0);
				if(i<this.list.length-1&&this.index != i){
					this.dom.childNodes[1-this.activeI].src = this.list[i+1];
				};
				this.index = i;
...
var canplayLis=function(e){canplay.call(e.target.link,e);mv.removeEventListener('canplay',canplayLis)}
			...
			mv.addEventListener('loadedmetadata',function(e){if(e.target._currentTime){e.target.currentTime=e.target._currentTime;e.target._currentTime=null}});
...
//			mv.addEventListener('stalled',function(e){onwaiting.call(this.link,e)});
			...
			if(list.length>1){
				...
				m_v.addEventListener('loadedmetadata',function(e){if(e.target._currentTime){e.target.currentTime=e.target._currentTime;e.target._currentTime=null}});
...
//				m_v.addEventListener('stalled',function(e){onwaiting.call(this.link,e)});
		...
		this.currentTimes=function(){
			return Bar_Controller.currentTimes();
		}
	};
