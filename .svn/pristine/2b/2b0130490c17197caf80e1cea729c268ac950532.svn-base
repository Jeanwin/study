define([ 'app', 'config' ], function(app, config) {
	...
	app.controller(
			'MainCtrl',
			[ '$scope', '$http', '$translate', '$timeout', '$interval', '$location', '$cookies','VideoService', 'CodeService', 'SecurityService', '$q', '$modal', '$filter',
					function($scope, $http, $translate, $timeout, $interval, $location, $cookies, VideoService,codeService, SecurityService, $q, $modal, $filter) {
						$scope.showVideo = function(url, isClose) {
							VideoService.showVideo(url, isClose);
						}
						// 关于系统
						...
						$scope.getCodes = function(keywords, user) {
							...
							codeService.getCodes(keywords, user).then(function(data) {
						...
						$scope.$watch('sourceList', function() {
							...
							var _temp = $filter('filter')($scope.sourceList, {
								checked : true
							});
						...
						$scope.fillSearchInput = function(index, scheduletype) {
							$scope.focus = true;
							var _temp = $filter('filter')($scope.schedules, {
						...
						var init = function() {
							...
							/*
							 * $http.get("data/releaseds.list.json").success(function(response) {
							...
							$scope.Loginhidden = false;
							/*
							 * if (readUserFromCookie()) { startTimer(); } else {
							 * window.location.href = 'login.jsp'; }
							 */
