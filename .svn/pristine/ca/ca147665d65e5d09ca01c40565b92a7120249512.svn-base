...
var deptTreeNodes;
var checedDept={id:'',title:''};
var makeDeptTree = function() {
	$.ajax({
		url : '/study/dept/getDeptTree',
		type : 'GET',
		async : false,
		datatype : 'json',
		cache : true,
			...
			if (data != null && data != undefined) {
				...
				function trans(nodes) {
					...
					$.each(nodes, function(i, d) {
						...
						htm += d.title + '</span>';
						htm += '<span class="hidden">' + JSON.stringify({id:d.id,title:d.title}) + '</span>';
						htm += '</li>';
						if (d.nodes != null) {
							htm += trans(d.nodes);
						}
				...
				if (trees.nodes.length > 0) {
					html += '<ul><li><i style="color: #2196F3;margin-right:10px" class="glyphicon glyphicon-folder-close"></i><span id="dept_name" style="display:inline-block;">' + trees.title + '</span> <span class="hidden" id="dept">' + JSON.stringify({id:trees.id,title:trees.title}) + '</span></li>';
...
var fillForm = function() {
	if (editMode) {
		$.ajax({
			url : '/study/private/getActiveEdit',
			type : 'POST',
			async : false,
			dataType : "json",
			data : $.param({
...
var imgPreview = function() {
	...
	if (suffix != 'jpg' && suffix != 'gif' && suffix != 'jpeg' && suffix != 'png') {
		...
		document.getElementById('active_img').src = '/study/static/img/front-cover.jpg';
		$("#active_file").val('');
...
var delReviewerCheck = function(userId){
	...
	$.ajax({
		type : "POST",
		data: JSON.stringify({userId:userId,activeId:$.getUrlParam("activeid")}),
		async : false,
		url : "/study/private/delReviewerCheck.do",
		dataType : "json",
...
var getUsersByDept = function(pageIndex) {
	...
	$.ajax({
		...
		async : true,
		url : "/study/private/getByDept.do",
		dataType : "json",
		cache : false,
		contentType : 'application/json; charset=UTF-8',
		success : function(data) {
...
var searchSpecialist = function(activeid) {
	...
	$.ajax({
		...
		async : false,
		cache : false,
		url : "/study/private/reviewUsers.do",
		dataType : "json",
		contentType : 'application/json; charset=UTF-8',
		success : function(data) {
...
var submitActive = function() {
	...
	if (fileName != null && fileName != '') {
		...
		var _url = '/study/private/newActiveHasPicture.do';
		if (editMode) {
			_url = '/study/private/updateActiveHasPicture';
		}
		;
			...
			$.ajaxFileUpload({
				...
				success : function(rd, status) {
					...
					if (rd.id === '1') {
						...
						setTimeout(function() {
							if (editMode) {
								window.location = "/active_detail.html?activeid=" + $.getUrlParam("activeid");
							} else {
								window.location = "/active_list.html";
							}
						}, 2000);
					} else {
		...
		if (validateNewActive(data)) {
			var _aurl = "/study/private/newActiveHasNoPicture.do";
			if (editMode) {
				_aurl = '/study/private/updateActiveHasNoPicture';
			}
			;
			$.ajax({
				type : "POST",
...
var getActiveType = function() {
	...
	var options = "";
	var nav = "<li role='presentation'><a href='./active_list.html' style='background-color:#4F9FEB'>全部活动</a></li>";
	if (activeType != null) {
		$.each(activeType, function(i, d) {
			if (i == 0) {
				options += "<option value='" + d.value + "' selected='true'>" + d.name + "</option>";
				//nav += "<li role='presentation'><a href='./active_list.html'>" + d.name + "</a></li>";
			} else {
				options += "<option value='" + d.value + "'>" + d.name + "</option>";
			}
			nav += "<li role='presentation'><a href='./c_"+(i+1)+"/active_list.html'>" + d.name + "</a></li>";
		});
	}
...
var getReviewOptions = function() {
	$.ajax({
		url : "/study/gateway/getReviewOptions",
		type : "POST",
		dataType : "json",
		success : function(data) {
