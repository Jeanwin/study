...
// }
	    ...
	    var fp=new FlexPaperViewer( 
		         '../static/js/curriculum/flexpaper/FlexPaperViewer',         
		         aid, {  
    	...
    	$.ajax({
    		...
    		async : false,
    		url : "selectCurriculumDetails.do",
    		dataType : "json",
...
// document.getElementById("datestarttime").innerHTML=data.data.date;
    			...
    			if(currentUser == undefined ||(data.data.loginname != currentUser.loginname)){
    				$("#editmaterials").prop('disabled',true);
    			}else{
    				$("#editmaterials").prop('disabled',false); 
    			}
    		}
    	});
    	...
    	$("#savenote").click(function(){
    		...
    		if($("#typedata").html() == '1'){
        		...
        		var minute=(myVideo.currentTime.toString().split(".")[0]/60).toString().split(".")[0];
        		var s=Math.ceil(('0.'+(myVideo.currentTime.toString().split(".")[0]/60).toString().split(".")[1])*60);
        		
        		if(s.toString().length == 1){
        			videotime=minute+':0'+s;
        		}else{
        			videotime=minute+':'+s;
        		}
    		}
        	...
        	$.ajax({
        		...
        		data : JSON.stringify(data),
        		async : false,
        		url : "../rest/curriculumDetails/addNote.do",
        		dataType : "json",
        		contentType : 'application/json; charset=UTF-8',
        		success : function(data) {
        			getNoteList();
    		...
    		if(workid != null && workid != undefined){
    			if(myReview == null || myReview == undefined){
    	    		...
    	    		$.ajax({
    	    	 		...
    	    	 		data : JSON.stringify(data),
    	    	 		async : false,
    	    	 		url : "../rest/curriculumDetails/savetReviewDetails.do",
    	    	 		dataType : "json",
    	    	 		contentType : 'application/json; charset=UTF-8',
    	    	 		success : function(data) {
 ...
 var getReviewByWorkId = function(){
		...
		$.ajax({
			type : "POST",
	 		data : $.param({workid:workId}),
	 		async : false,
	 		url : "../private/getReviewByUser.do",
	 		dataType : "json",
	 		//contentType : 'application/json; charset=UTF-8',
	 		success : function(data) {
...
// addMessage("error", data);
 ...
 var updateNote=function(id,content){
	 ...
	 $.ajax({
	 		...
	 		data : JSON.stringify(data),
	 		async : false,
	 		url : "../rest/curriculumDetails/updateNote.do",
	 		dataType : "json",
	 		contentType : 'application/json; charset=UTF-8',
	 		success : function(data) {
	 			getNoteList();
...
// addMessage("error", data);
 ...
 var deleteNote=function(id){
	 ...
	 $.ajax({
	 		...
	 		data : JSON.stringify(data),
	 		async : false,
	 		url : "../rest/curriculumDetails/deleteNote.do",
	 		dataType : "json",
	 		contentType : 'application/json; charset=UTF-8',
	 		success : function(data) {
	 ...
	 $.ajax({
	 		...
	 		success : function(data) {
	 			...
	 			if(typedata == '1'){
	 				...
	 				videohtml +="<div class='mainWindow'>";
	 				if(data.data[0].video0 != null){
	 					videohtml +="<video id='MainVideo' src="+data.data[0].video0+" style='width:100%;height:100%;background-color: black;' controls autoplay></video> ";  	
	 				}
	 				videohtml +=" </div>";
	 				videohtml +=" <div class='childWindow'>";
	 				if(data.data[0].video1 != null){
	 					videohtml += "<div class='c1'>";
	 					videohtml +=   "<div class='videoPos'>";
	 					videohtml +=    "<video id='MainVideo2' src="+data.data[0].video1+" style='width:100%;height:100%;background-color: black;'  autoplay muted></video>";
	 					videohtml +=  "</div>";       
	 					videohtml +=  "</div>";
	 				}
	 				if(data.data[0].video2 != null){
	 					videohtml += "<div class='c2'>";
	 					videohtml +=   "<div class='videoPos'>";
	 					videohtml +=    "<video id='MainVideo3' src="+data.data[0].video2+" style='width:100%;height:100%;background-color: black;'  autoplay muted></video>";
	 					videohtml +=  "</div>";       
	 					videohtml +=  "</div>";
	 				}
	 				if(data.data[0].video3 != null){
	 					videohtml += "<div class='c3'>";
	 					videohtml +=   "<div class='videoPos'>";
	 					videohtml +=    "<video id='MainVideo4' src="+data.data[0].video3+" style='width:100%;height:100%;background-color: black;'  autoplay muted></video>";
	 					videohtml +=  "</div>";       
	 					videohtml +=  "</div>";
	 				}
	 				videohtml +=" </div>";
	 			...
	 			//当前音量
	 			var currentvolume;
	 			if($("#MainVideo")[0] != undefined){
	 				//获取时长
		 			$("#MainVideo")[0].addEventListener("loadedmetadata",function(e){
		 			...
		 			//播放
		 			$("#MainVideo")[0].addEventListener("play",function(e){
		 		    	var currenttime = e.target.currentTime;
		 		    	var id = window.location.search+"";
		 		    	id = id.substring(id.indexOf("=")+1);
		 		    	play("2", id, currenttime);
		 		     });
		 			//暂停
		 			$("#MainVideo")[0].addEventListener("pause",function(e){
		 		    	var currenttime = e.target.currentTime;
		 		    	var id = window.location.search+"";
		 		    	id = id.substring(id.indexOf("=")+1);
		 		    	pause("2", id, currenttime);
		 		     });
		 			//音量改变
		 			
		 			$("#MainVideo")[0].addEventListener("volumechange",function(e){
		 		    	var volume = e.target.volume;
		 		    	var id = window.location.search+"";
		 		    	id = id.substring(id.indexOf("=")+1);
		 		    	changeVolume("2", id, currentvolume,volume);
		 		    	currentvolume = volume;
		 		     });
		 			//视频错误
		 			$("#MainVideo")[0].addEventListener("error",function(e){
		 		    	var error = e.target.error;
		 		    	var errorcode = e.target.error.code;
		 		    	var id = window.location.search+"";
		 		    	id = id.substring(id.indexOf("=")+1);
		 		    	error("2", id, error,errorcode);
		 		     });
		 			//播放结束
		 			$("#MainVideo")[0].addEventListener("ended",function(e){
		 				stopGoal();
		 		     });
	 			}
...
function submitComment(id){
	...
	if(content!=""){
    	...
    	$.ajax({
    		...
    		data : JSON.stringify(data),
    		async : false,
    		url : "../rest/curriculumDetails/addComment.do",
    		dataType : "json",
    		contentType : 'application/json; charset=UTF-8',
    		success : function(data) {
    			selectCommentCount();
			 ...
			 */
		    	...
		    	$("#savetRecordDetails").click(function(){
		    		...
		    		$.ajax({
		    	 		...
		    	 		data : JSON.stringify(data),
		    	 		async : false,
		    	 		url : "../rest/curriculumDetails/savetRecordDetails.do",
		    	 		dataType : "json",
		    	 		contentType : 'application/json; charset=UTF-8',
		    	 		success : function(data) {
...
// data : JSON.stringify(data),
		async : false,
		url : "../rest/curriculumDetails/getTrees.do",
		dataType : "json",
		contentType : 'application/json; charset=UTF-8',
		success : function(data) {
			var div="";
...
function importMaterial(){
	...
	$.ajax({
		...
		data : JSON.stringify(data),
		async : false,
		url : "../rest/curriculumDetails/importMaterial.do",
		dataType : "json",
		contentType : 'application/json; charset=UTF-8',
		success : function(data) {
			$(".file-box").hide();
...
function uploadOnClick() {
    ...
    var FileController = "../rest/curriculumDetails/uploadMaterial.do";                    // 接收上传文件的后台地址
...
// $(this).parent().parent().find("td[id='materialid']").val();
 		    		...
 		    		$.ajax({
 		    	 		...
 		    	 		data : JSON.stringify(data),
 		    	 		async : false,
 		    	 		url : "../rest/curriculumDetails/updateMaterial.do",
 		    	 		dataType : "json",
 		    	 		contentType : 'application/json; charset=UTF-8',
 		    	 		success : function(data) {
 		    	 			addMessage("success", "修改成功！");
	...
	$.ajax({
 		...
 		success : function(data) {
 			if(data.data.length >0){
 				...
 				$.each(data.data, function(i, note) {
 					if(i==0){
 					noteviewone +="<div><span class='pull-right load'><a href='../gateway/down?path="+note.fileurl+"&name="+note.name+"'><i class='icon-my-download'></i>下载</a></span><i class='icon-"+note.nametype+"'></i> "+note.name+"</div>";
 					noteviewone+="<div class='bookContent'>" +
 							"<a id='viewerPlaceHolder' style='width:890px;height:750px;display:block'></a>" +
 							"</div>";
 				...
 				$.each(data.data, function(i, noteother) {
 					...
 					if(noteother.flag == '1'){
 						noteviewother +=		"<span class='load' ><a href='../gateway/down?path="+noteother.fileurl+"&name="+noteother.name+"'><i class='icon-my-download'></i>下载</a></span>";
 					}
 					...
 					"<a id='viewerPlaceHolder"+i+"' style='width:890px;height:750px;display:block'></a><span style='display:none;'>"+noteother.transferurl+"</span>" +
 							"</div>";
 					noteviewother +=" </td>";
 					noteviewother +=" </tr>";
...
function deleteMaterial(id){
	...
	$.ajax({
 		...
 		data : JSON.stringify(data),
 		async : false,
 		url : "../rest/curriculumDetails/deleteMaterial.do",
 		dataType : "json",
 		contentType : 'application/json; charset=UTF-8',
 		success : function(data) {
 			addMessage("success", "删除成功！");
...
function allowDownloadMaterial(id){
	...
	$.ajax({
 		...
 		data : JSON.stringify(data),
 		async : false,
 		url : "../rest/curriculumDetails/allowDownloadMaterial.do",
 		dataType : "json",
 		contentType : 'application/json; charset=UTF-8',
 		success : function(data) {
 			selectMaterialList();
...
// alert($("#templatename").val());
	...
	$.ajax({
 		...
 		data : JSON.stringify(data),
 		async : false,
 		url : "selectReviewOptiondetail.do",
 		dataType : "json",
 		contentType : 'application/json; charset=UTF-8',
 		success : function(data) {
 			console.log("评审明细" + data.ReviewTemplate);
