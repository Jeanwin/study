			...
			if (d.total > 0) {
				...
				$.each(d.data, function(i, work) {
					...
					if (work.picture != null && work.picture != undefined && work.picture != null) {
						...
						worksView += "<td><input value='" + work.id + "' type=\"checkbox\" name=\"personExpert\"><img src='/study/static/img/front-cover.jpg' width='253' height='165'/>" + work.name
								+ "</td>";
					}
					;
...
var assignSpecialist = function(selectWorks, selectExpert) {
	...
	$.ajax({
		url : '/study/private/assignSpecialist.do',
		type : 'POST',
		cache : false,
		dataType : 'json',
		contentType : 'application/json;charset=UTF-8',
...
var queryActiveDetail = function() {
	...
	$.ajax({
		url : '/study/gateway/getActiveById',
		type : 'POST',
		dataType : 'json',
		async : false,
		// contentType : 'application/json; charset=UTF-8',
		...
		success : function(da) {
			...
			if (da != null && da != undefined) {
				...
				if (da.picture != null && da.picture != undefined && da.picture != '') {
					...
					$("#active_img").attr("src", "/study/static/img/front-cover.jpg");
				}
				;
				if (da.status === '未开始') {
		...
		success : function(d) {
			...
			if (d.total > 0) {
				...
				$.each(d.data, function(i, work) {
					...
					// 1直播，2 录像
					if (work.type == 1) {
						worksView += "<a href='/course_detail.html?curriculumid=" + work.resourceid + "&workid=" + work.id + "'>";
					} else {
						worksView += "<a href='/course_detail.html?workid=" + work.id + "&source=" + work.resourceSource + "'>";
					}
					if (work.picture != null && work.picture != undefined && work.picture != null) {
						worksView += "<img src='" + work.picture + "' width='253' height='165'/>";
					} else {
						worksView += "<img src='/study/static/img/front-cover.jpg' width='253' height='165' />";
					}
					;
					// 1直播，2 录像
					if (work.type == 1) {
						worksView += "</a></div><h3><a href='/course_detail.html?curriculumid=" + work.resourceid + "&workid=" + work.id + "'>" + work.name + "</a></h3>";
					} else {
						worksView += "</a></div><h3><a href='/course_detail.html?workid=" + work.id + "&source=" + work.resourceSource + "'>" + work.name + "</a></h3>";
					}
...
var getWorksWithReview = function(pageIndex) {
	...
	console.log(data);
	$.ajax({
		url : '/study/private/getReviewList',
		type : 'POST',
		dataType : 'json',
		async : false,
		cache : false,
			...
			if (d.total > 0) {
				...
				for (var i = 0; i < d.data.length; i++) {
					...
					if (d.data[i].picture != null && d.data[i].picture != undefined && d.data[i].picture != '') {
						...
						trs += "<td><img src='/study/static/img/lemon-pages.jpg' width='62' height='45'/>" + d.data[i].name + "</td>"
					}
					;
...
var getBarChart = function() {
	...
	$.ajax({
		url : '/study/private/getSignerDept',
		type : 'POST',
		dataType : 'html',
...
var getReviewUserDetail = function() {
	...
	$.ajax({
		url : '/study/private/getSpecialDetail.do',
		type : 'POST',
		dataType : 'json',
		// contentTpe : 'application/json;charset=UTF-8',
		cache : false,
...
var getDetailByWorkId = function(worksid) {
	...
	$.ajax({
		url : '/study/private/reviewDetail',
		type : 'POST',
		cache : false,
		dataType : 'json',
		contentTpe : 'application/json;charset=UTF-8',
...
var imgPreview = function() {
	...
	if (suffix != 'jpg' && suffix != 'gif' && suffix != 'jpeg' && suffix != 'png') {
		...
		document.getElementById('works_img').src = '/study/static/img/front-cover.jpg';
		$("#works_file").val('');
...
var okSelectFilm = function() {
	...
	if (curNode != null && curNode != undefined) {
		...
		if (curNode.imageurl != null && curNode.imageurl != undefined) {
			...
			$("#works_img").attr("src", "/study/static/img/front-cover.jpg");
		}
		//
...
var getBroadCast = function(pageIndex) {
	...
	$.ajax({
		url : "/study/private/liveCurriculum.do",
		type : "POST",
		dataType : "json",
		contentType : "application/json;charset=UTF-8",
		data : JSON.stringify(data),
...
var makeVideoTree = function() {
	initCheckedVideo();
	$.ajax({
		url : "/study/private/videoTrees.do",
		type : "POST",
		async : false,
		dataType : "json",
		success : function(d) {
...
var submitWorks = function() {
	if (currentUser.usertype == '1') {
		...
		if (validateInsertWorks(data) && hasImage && isImage) {
			// 有图片上传
			$.ajaxFileUpload({
				url : '/study/private/signUpHasPicture.do',// 处理图片
				secureuri : false,
			...
			// 没有图片上传
			$.ajax({
				url : "/study/private/signUpHasNoPicture.do",
				type : "POST",
				cache : false,
				dataType : "json",
				contentType : "application/json;charset=UTF-8",
...
var onSelectFilm = function() {
	...
	if ($("#choose").val() != null && $("#choose").val() != '' && $("#choose").val() != undefined) {
		...
		if ($.inArray(suffix, videoSufix) == -1) {
			...
			// '/study/static/img/front-cover.jpg';
			$("#choose").val('');
...
var openEditActive = function() {
	...
	if (currentUser.roles.indexOf("系统管理员") != -1) {
		window.location = "/new_active.html?activeid=" + activeid;
		/*
		 * if (currentActive.status === "未开始" || currentActive.status ===
		 * "报名中,评审未开始" || currentActive.status === "报名已结束,评审未开始") {
		 * window.location = "/study/private/editActive?activeid=" + activeid; }
